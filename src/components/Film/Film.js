...
class Film extends React.Component {
  ...
  async componentDidMount() {
    const response = await fetch(
      `https://api.themoviedb.org/3/movie/${this.state.film_data[0].match.params.id}?api_key=526ca44fbbdbdd581bb4a6d9f1f87e15&append_to_response=videos`
    );
    ...
    if (json.poster_path) {
      this.setState({
        film_image: `https://image.tmdb.org/t/p/w342${json.poster_path}`
      });
    } else {
    ...
    if (json.backdrop_path) {
      this.setState({
        film_bg: `https://image.tmdb.org/t/p/w342${json.backdrop_path}`
      });
    } else {
              ...
              {this.state.loaded ? (
                <div className="info">
                  ...
                  <Link
                    className="title"
                    to={{
                      pathname: `/Watch/${this.state.film.id}`,
                      type: "Film",
                      backgraund: this.state.film_bg
                    }}
                  >
